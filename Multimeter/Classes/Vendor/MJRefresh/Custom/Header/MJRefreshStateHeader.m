...
- (void)setLastUpdatedTimeKey:(NSString *)lastUpdatedTimeKey
{
    ...
    if (lastUpdatedTime) {
        ...
        if ([cmp1 day] == [cmp2 day]) { // 今天
            formatter.dateFormat = [NSString stringWithFormat:@"%@ HH:mm", VTLOCALIZEDSTRING(@"今天")];
        ...
        // 3.显示日期
        self.lastUpdatedTimeLabel.text = [NSString stringWithFormat:@"%@%@", VTLOCALIZEDSTRING(@"最后更新："), time];
    } else {
        self.lastUpdatedTimeLabel.text = VTLOCALIZEDSTRING(@"最后更新：无记录");
    }
}
