...
#import "VTMeasurePresenter_8238.h"
...
@interface VTMeasurePresenter_8238()<VTSlotAlertViewDelegate>

@property (weak, nonatomic) UIView *contentView;
@property (strong, nonatomic) VTMeasureView_8238 *view;
@property (strong, nonatomic) VTSlotAlertView *alertView;
@property (assign, nonatomic) NSInteger lastPointPos;

@end

@implementation VTMeasurePresenter_8238
...
- (void)popAlertIfNeed:(NSInteger)pos {
    ...
    if (lastType != type) {
        ...
        if (type==1)
            imageName = @"8238_1.gif";
        else
            imageName = @"8238_2.gif";
        // 正在显示表格,则不弹窗
...
- (UIView *)view {
    if (!_view) {
//        _view =  [[[NSBundle mainBundle] loadNibNamed:@"VTMeasureView_8238" owner:self options:nil]lastObject];
        _view = [VTMeasureView_8238 new];
    }
    return _view;
}
