...
- (void)initChart{
    if(!self.isInitialized){
        ...
        if(self.padding != nil){
            ...
            self.paddingLeft   = [[self.padding objectAtIndex:1] floatValue];
            self.paddingBottom = [[self.padding objectAtIndex:2] floatValue];
            self.paddingRight  = [[self.padding objectAtIndex:3] floatValue];
        }
    ...
    CGContextSetFillColorWithColor(context, kChartBgColor.CGColor);
...
-(void)drawYAxis {
    ...
    for(int secIndex=0;secIndex<self.sections.count;secIndex++){
        ...
        for(int aIndex=0;aIndex<sec.yAxises.count;aIndex++){
            ...
            float middleValue = yaxis.tickInterval/2;
            ...
            if ([yAxisType isEqualToString:kFenShiLine]) {
                ...
                //显示分时图原点处左侧价格刻度
                [[@"" stringByAppendingFormat:format,yaxis.baseValue] drawInRect:CGRectMake(0, baseY - kYFontSizeFenShi, sec.frame.origin.x + sec.paddingLeft - 1, kYFontSizeFenShi * 2) withAttributes:@{NSFontAttributeName:[UIFont fontWithName:KYFontName size:kYFontSizeFenShi],NSParagraphStyleAttributeName:style,NSForegroundColorAttributeName:kFenShiYColor}];
            ...
            for(int i=1; i<= yaxis.tickInterval;i++){
                ...
                if(yaxis.baseValue + i*step <= yaxis.max && yaxis.max < MAXFLOAT){
                    ...
                    UIColor *textColor = kFenShiYColor;
